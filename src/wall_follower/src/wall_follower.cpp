...
int main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "wall_follower"); //changed the node name from "node" to "wall_follower_node"
    ...
    motor_command_publisher = n.advertise<geometry_msgs::Twist>("/cmd_vel_mux/input/teleop", 100);  // letting the topic name to be same as "/robotactuate"

    // Subscribe to the /scan topic and call the laser_callback function
    laser_subscriber = n.subscribe("/scan", 1000, laser_callback); // letting the topic name to be same i.e. "laserscan"
