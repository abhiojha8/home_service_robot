...
int main(int argc, char** argv){
  // Initialize the pick_object node
  ...
  ROS_INFO("Sending the pickup location");
  ac.sendGoal(goal);
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
    ROS_INFO("Hooray, the robot has reached the pickup location");
  else
  ...
  // Check if the robot reached its goal
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
    ROS_INFO("Hooray, the robot has reached the drop off location");
  else
